...
public class EventServiceImpl implements EventService {
    ...
    public EventFullDto createEvent(Long userId, NewEventDto newEventDto) {
        ...
        //проверить, что дата и время на которые намечено событие не раньше,
        // чем через два часа от текущего момента, иначе ошибка 400
    ...
    public EventFullDto getPublishedEventById(Long eventId, HttpServletRequest request) {
        ...
        statClient.addHit(HitDto.builder()
                ...
                .timestamp(LocalDateTime.now().format(formatter))
                .build());
        //снова получаем информацию из сервиса статистики по уникальному IP, если количество увеличилось,
        //то увеличиваем количество просмотров (views) у текущего события
    ...
    private Integer getCountHits(HttpServletRequest request) {
        log.info("Client ip: {}", request.getRemoteAddr());
        log.info("Endpoint path: {}", request.getRequestURI());
        //получаем информацию из сервиса статистики по количеству просмотров с уникального IP
